...
#%% NOVIDADE Definir 'callback'
callback = EarlyStopping(
    monitor='loss',  # métrica de desempenho a ser monitorada
    patience=5,  # número de épocas que o modelo deve esperar antes de parar o treinamento
    min_delta=0.005,  # diferença mínima que a métrica de desempenho deve ter para ser considerada uma melhoria
    verbose=1,  # nível de verborragia
    mode='min'  # modo de monitoramento (min ou max)
)

#%% TREINAMENTO DA REDE
model.fit(X, y, epochs=2000, callbacks=[callback], verbose=1)
